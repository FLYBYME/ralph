...
export class GitRunner {
  ...
  private async runCommand(command: string, args: string[]): Promise<{ stdout: string; stderr: string; exitCode: number }> {
    try {
      const { stdout, stderr } = await execFileAsync(command, args, { cwd: this.repoPath });
  ...
  public async createBranch(name: string, baseBranch: string): Promise<void> {
    await this.runCommand('git', ['checkout', baseBranch]);
    await this.runCommand('git', ['pull', 'origin', baseBranch]);
